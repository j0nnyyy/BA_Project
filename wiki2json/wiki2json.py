...
class Wiki2Json:
    def __init__(self):
        ...
        self._re_begin_page = re.compile(r'\s+<page>')
        self._re_end_page = re.compile(r'\s+</page>')
        self._re_begin_revision = re.compile(r'\s+<revision>')
        self._re_end_revision = re.compile(r'\s+</revision>')
    ...
    def parse_line(self, line):
        ...
        if not self._in_page:
            if self._re_begin_page.match(line):
                self._in_page = True
                self._revision_count = 0
        # </page>
        ...
        # between <page> and </page>
            ...
            '''
            # <revision>
            if not self._in_revision:
                if self._re_begin_revision.match(line):
    ...
    def _init_multiline_tag(self, tag):
        if tag == 'revision' and self._revision_count == 0:
            self._parent().start_array(tag)
            self._revision_count += 1
        elif tag == 'revision' and self._revision_count >= 1:
            self._parent().array_separator()
        else:
